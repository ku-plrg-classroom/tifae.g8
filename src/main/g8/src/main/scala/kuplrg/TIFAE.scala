...
case class PolyType(ks: List[Int], ty: Type):
  ...
  // the string form of a polymorphic type
  def str: String =
...
object PolyType:
  ...
  // conversion from type to polymorphic type
  def from(pair: (Type, Solution)): PolyType =
    val (ty, sol) = pair
    ...
    def subst(ty: Type): Type = ty match
      ...
      case _ => ty
    PolyType(ls, subst(ty.subst(sol)))
